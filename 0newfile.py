...
def logo(n):
   ...
   \033[1;92m██   ██ ██   ██      ██ ██   ██ ██  ██ ██ 
   \033[1;92m██   ██ ██   ██ ███████ ██   ██ ██   ████ 
   \033[1;96mArgentina Win Ty Free Dicci HASAN FIRE 🔥💗
                 Stetus- Trail{H}•{K}•{M}•""")   
def logo2():
	return str(f"""
   \033[1;92m██   ██  █████  ███████  █████  ███    ██ 
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada HASAN💔 cp")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada HASAN💔 ok")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_HASAN():
	...
	if one in ['1','01']:
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada HASAN💚 ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		except:sys.exit(f" [{M}>{P}] tidak HASAN💔 cp")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('CP/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada HASAN💔 cp")
		print(kk+buka+P)
	else:sys.exit(f" [{M}>{P}] isi yang benar")
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='Lihat HASAN💔 Selanjutnya').get('href')
		if(link):
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
